...
$("btnPng").onclick = async () => {
  ...
  try {
    ...
    try {
      ...
      await new Promise<void>((res, rej) => { image.onload = () => res(); image.onerror = () => rej(new Error("画像化に失敗")); image.src = svgUrl; });
      const c = document.createElement("canvas"); c.width = W * 2; c.height = H * 2;
      const x = c.getContext("2d") as CanvasRenderingContext2D; x.scale(2, 2);
