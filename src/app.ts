...
interface Shift { id: string; name: string; time: string; need: number; needLeader: number; stationId: string; }
interface AppState {
  period: { start: string; end: string };
  stations: Station[];
  members: Member[];
  shifts: Shift[];
  leaves: string[]; // ["memberId|YYYY-MM-DD"]
  avail: Record<string, string>; // ["memberId|YYYY-MM-DD"] = "HH:MM-HH:MM" 出勤可能時間。空/未設定=終日可
  result: Record<string, string[]>; // {"YYYY-MM-DD|shiftId": [memberId]}
}
...
let state: AppState;
try {
  state = (JSON.parse(localStorage.getItem(KEY) as string) as AppState) || defaultState();
} catch { state = defaultState(); }
const save = (): void => localStorage.setItem(KEY, JSON.stringify(state));
const leaveSet = (): Set<string> => new Set(state.leaves);
...
const wday = (iso: string): string => "日月火水木金土"[new Date(iso + "T00:00:00").getDay()];
const md = (iso: string): string => iso.slice(5).replace("-", "/");
...
function hourOpts(sel: string): string {
  ...
  for (let i = 0; i < 24; i++) o += `<option value="${i}:00"${i === h ? " selected" : ""}>${i}時</option>`;
  return o;
}
// 希望時間とシフト枠の重なり分数。希望なし=枠全体、枠の時刻不明=0(全員同列)
function overlapMin(memberId: string, date: string, shiftTime: string): number {
...
function fitsAvail(memberId: string, date: string, shiftTime: string): boolean {
  if (!parseRange(shiftTime)) return true;
...
function autoAssign(): void {
  const ds = dates(), counts: Record<string, number> = {}, result: Record<string, string[]> = {};
  state.members.forEach((m) => (counts[m.id] = 0));
  const leaves = leaveSet();
  let prevDay = new Set<string>();
  for (const date of ds) {
    const assignedToday = new Set<string>();
    for (const s of state.shifts) {
      const key = date + "|" + s.id;
      const cand = (onlyLeader: boolean): Member[] =>
        state.members
...
// --- イベント (委譲で最小化)
$("startDate").onchange = (e) => { state.period.start = (e.target as HTMLInputElement).value; save(); renderLeave(); renderResult(); };
$("endDate").onchange = (e) => { state.period.end = (e.target as HTMLInputElement).value; save(); renderLeave(); renderResult(); };
$("btnAddMember").onclick = () => {
  const v = $input("memberName").value.trim(); if (!v) return;
  state.members.push({ id: uid(), name: v, isLeader: false, stationIds: state.stations.map((t) => t.id) });
  $input("memberName").value = ""; save(); renderAll();
};
$("btnAddStation").onclick = () => {
  const v = $input("stationName").value.trim(); if (!v) return;
  const id = uid();
  ...
  $input("stationName").value = ""; save(); renderAll();
};
$("stationList").onclick = (e) => {
  const t = tgt(e);
  if (t.dataset["act"] !== "del") return;
  const id = (t.closest("[data-id]") as HTMLElement | null)?.dataset["id"]; if (!id) return;
  state.stations = state.stations.filter((x) => x.id !== id);
  const first = state.stations[0]?.id || "";
  state.shifts.forEach((s) => { if (!state.stations.some((x) => x.id === s.stationId)) s.stationId = first; });
  state.members.forEach((m) => (m.stationIds = (m.stationIds || []).filter((x) => x !== id)));
  save(); renderAll();
};
$("stationList").onchange = (e) => {
  const t = tgt(e);
  if (t.dataset["k"] !== "name") return;
  const st = state.stations.find((x) => x.id === (t.closest("[data-id]") as HTMLElement | null)?.dataset["id"]); if (!st) return;
  st.name = (t as HTMLInputElement).value; save(); renderAll();
};
$("btnAddShift").onclick = () => {
...
$("memberList").onclick = (e) => {
  const t = tgt(e);
  if (t.dataset["act"] !== "del") return;
  const row = t.closest(".mrow") as HTMLElement | null; if (!row?.dataset["id"]) return;
  const m = memberById(row.dataset["id"] as string); if (!m) return;
...
$("memberList").onchange = (e) => {
  ...
  if (t.dataset["k"] === "isLeader") m.isLeader = inp.checked;
  else if (t.dataset["k"] === "name") m.name = inp.value;
  else if (t.dataset["k"] === "station") {
    m.stationIds ||= [];
    if (inp.checked) { if (!m.stationIds.includes(inp.value)) m.stationIds.push(inp.value); }
    else m.stationIds = m.stationIds.filter((id) => id !== inp.value);
  }
  else return;
  save(); renderAll();
};
$("shiftList").onclick = (e) => {
  const t = tgt(e);
  if (t.dataset["act"] !== "del") return;
  const row = t.closest(".srow") as HTMLElement | null; if (!row) return;
...
$("shiftList").onchange = (e) => {
  ...
  if (t.dataset["k"] === "name") s.name = v;
  else if (t.dataset["k"] === "timeFrom" || t.dataset["k"] === "timeTo") {
    const f = (row.querySelector('select[data-k="timeFrom"]') as HTMLSelectElement | null)?.value || "";
    const tt = (row.querySelector('select[data-k="timeTo"]') as HTMLSelectElement | null)?.value || "";
    s.time = f || tt ? `${f}-${tt}` : "";
  }
  else if (t.dataset["k"] === "stationId") s.stationId = v;
  else if (t.dataset["k"] === "need") s.need = Math.max(1, +v || 1);
  else if (t.dataset["k"] === "needLeader") s.needLeader = Math.max(0, +v || 0);
  else return;
  save(); renderShifts(); renderResult();
};
$("leaveTable").onclick = (e) => {
  const btn = tgt(e).closest("[data-off]") as HTMLElement | null; if (!btn) return;
...
$("leaveTable").onchange = (e) => {
  ...
  if (!inp.dataset["av"] || !inp.dataset["m"] || !inp.dataset["d"]) return;
...
($("resultTable").onchange = $("resultTable").onclick = (e: Event) => {
  const t = tgt(e);
  if (t.dataset["rm"]) {
    const { rm, d, s } = t.dataset; if (!rm || !d || !s) return;
    state.result[d + "|" + s] = (state.result[d + "|" + s] || []).filter((id) => id !== rm);
    save(); renderResult();
  } else if (t.dataset["add"] !== undefined && (t as HTMLSelectElement).value) {
    const { d, s } = t.dataset; if (!d || !s) return;
    (state.result[d + "|" + s] ||= []).push((t as HTMLSelectElement).value);
    save(); renderResult();
  }
});
...
interface CellText { top: string; bottom: string; short: boolean; }
function cellText(d: string, s: Shift): CellText {
  const ids = state.result[d + "|" + s.id] || [];
  const names = ids.map((id) => memberById(id)?.name || "").filter(Boolean).join("・") || "不足";
  const miss = s.need - ids.length;
  return { top: names, bottom: miss > 0 ? `要${miss}` : "", short: miss > 0 };
}
interface TableLayout { widths: number[]; headerH: number; rowH: number; W: number; H: number; }
function tableLayout(measure: (t: string) => number): TableLayout {
  const ds = dates();
...
function drawTable(ctx: CanvasRenderingContext2D, L: TableLayout): void {
  const ds = dates();
  ctx.font = PNG_FONT; ctx.textBaseline = "middle"; ctx.textAlign = "center";
  ctx.fillStyle = "#fff"; ctx.fillRect(0, 0, L.W, L.H);
  const xs: number[] = [PNG_PX];
  L.widths.forEach((w) => xs.push(xs[xs.length - 1] + w));
  const cx = (c: number) => (xs[c] + xs[c + 1]) / 2;
  // ヘッダ
  ctx.fillStyle = "#eee"; ctx.fillRect(PNG_PX, PNG_PT, L.W - PNG_PX * 2, L.headerH);
  ctx.fillStyle = "#000";
  ...
  ds.forEach((d, r) => {
    ...
    state.shifts.forEach((s, i) => {
      ...
      if (ct.bottom) ctx.fillText(ct.bottom, cx(i + 1), y + 26);
    });
  });
  // 罫線
  ctx.strokeStyle = "#999"; ctx.lineWidth = 1; ctx.beginPath();
  xs.forEach((x) => { ctx.moveTo(x, PNG_PT); ctx.lineTo(x, L.H - PNG_PB); });
  ctx.moveTo(PNG_PX, PNG_PT); ctx.lineTo(L.W - PNG_PX, PNG_PT);
  ctx.moveTo(PNG_PX, PNG_PT + L.headerH); ctx.lineTo(L.W - PNG_PX, PNG_PT + L.headerH);
  ds.forEach((_, r) => {
    const y = PNG_PT + L.headerH + (r + 1) * L.rowH;
    ctx.moveTo(PNG_PX, y); ctx.lineTo(L.W - PNG_PX, y);
  });
  ctx.stroke();
}
// 遷移・data URLなし。進捗と失敗理由はあらかじめ用意した枠内に表示する。
$("btnPng").onclick = async () => {
  const card = $("pngCard"), img = $("pngImg") as HTMLImageElement, msg = $("pngMsg");
  const say = (t: string) => { msg.textContent = t; };
  card.hidden = false;
  try {
    say("生成中…");
    const c = document.createElement("canvas");
    const meas = c.getContext("2d") as CanvasRenderingContext2D;
    meas.font = PNG_FONT;
    const L = tableLayout((t) => meas.measureText(t).width);
    c.width = L.W * 2; c.height = L.H * 2; // 2倍解像度
    const ctx = c.getContext("2d") as CanvasRenderingContext2D;
    ctx.scale(2, 2);
    drawTable(ctx, L);
    // 文字と罫線のみの描画のため汚染なし。全ブラウザでtoBlob可。
    const blob = await new Promise<Blob | null>((res) => c.toBlob(res, "image/png"));
    if (!blob) throw new Error("PNG化に失敗");
    ...
    try {
      ...
      if (navigator.canShare?.({ files: [file] })) { await navigator.share({ files: [file], title: "シフト表" }); say("共有シートを開きました"); return; }
    } catch {}
    if (pngUrl) URL.revokeObjectURL(pngUrl);
    pngUrl = URL.createObjectURL(blob);
    img.src = pngUrl;
    say("画像を長押し → 「写真に追加」で保存できます");
  } catch (e) { say(`失敗: ${e instanceof Error ? e.message : String(e)}`); }
  card.scrollIntoView();
};
let pngUrl = "";
$("btnPngClose").onclick = () => {
